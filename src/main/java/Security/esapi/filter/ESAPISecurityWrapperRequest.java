...
public class ESAPISecurityWrapperRequest extends HttpServletRequestWrapper implements HttpServletRequest{
    private final Log logger = LogFactory.getLog(ESAPISecurityWrapperRequest.class);

    private String allowableContentRoot = "WEB-INF";

    public static final int HTTP_PARAMETER_VALUE_MAX = 10240;
    ...
    public String getContextPath() {
        ...
        String clean = "";
        try {
            clean = validator.getValidInput("HTTP context path: " + path, path, "HTTPContextPath", 150, false);
    ...
    public Cookie[] getCookies() {
        ...
        for (Cookie c : cookies) {
            ...
            try {
                ...
                String name = validator.getValidInput("Cookie name: " + c.getName(), c.getName(), "HTTPCookieName", 150, true);
                String value = validator.getValidInput("Cookie value: " + c.getValue(), c.getValue(), "HTTPCookieValue", 4096, true);
                ...
                if (domain != null) {
                    n.setDomain(validator.getValidInput("Cookie domain: " + domain, domain, "HTTPHeaderValue", 4096, false));
                }
                if (path != null) {
                    n.setPath(validator.getValidInput("Cookie path: " + path, path, "HTTPHeaderValue", 4096, false));
                }
                newCookies.add(n);
            } catch (ESAPIValidationException e) {
    ...
    public String getHeader(String name) {
        ...
        String clean = "";
        try {
            clean = validator.getValidInput("HTTP header value: " + value, value, "HTTPHeaderValue", 4096, true);
        } catch (ESAPIValidationException e) {
            // already logged
        }
        return clean;
    ...
    public Enumeration getHeaderNames() {
        ...
        while (en.hasMoreElements()) {
            try {
                ...
                String clean = validator.getValidInput("HTTP header name: " + name, name, "HTTPHeaderName", 150, true);
                v.add(clean);
            } catch (ESAPIValidationException e) {
                // already logged
            }
    ...
    public Enumeration getHeaders(String name) {
        ...
        while (en.hasMoreElements()) {
            try {
                ...
                String clean = validator.getValidInput("HTTP header value (" + name + "): " + value, value, "HTTPHeaderValue", 4096, true);
                v.add(clean);
            } catch (ESAPIValidationException e) {
                // already logged
            }
    ...
    public String getParameter(String name, boolean allowNull) {
        return getParameter(name, allowNull, HTTP_PARAMETER_VALUE_MAX, "HTTPParameterValue");
    }
    ...
    public String getParameter(String name, boolean allowNull, int maxLength, String regexName) {
        ...
        String clean = null;
        try {
            clean = validator.getValidInput("HTTP parameter name: " + name, orig, regexName, maxLength, allowNull);
        } catch (ESAPIValidationException e) {
            // already logged
        }
        return clean;
    ...
    public Map getParameterMap() {
        ...
        for (Object o : map.entrySet()) {
            try {
                ...
                String cleanName = validator.getValidInput("HTTP parameter name: " + name, name, "HTTPParameterName", 100, true);

                String[] value = (String[]) e.getValue();
                String[] cleanValues = new String[value.length];
                for (int j = 0; j < value.length; j++) {
                    String cleanValue = validator.getValidInput("HTTP parameter value: " + value[j], value[j], "HTTPParameterValue", HTTP_PARAMETER_VALUE_MAX, true);
    ...
    public Enumeration getParameterNames() {
        ...
        while (en.hasMoreElements()) {
            try {
                String name = (String) en.nextElement();
                String clean = validator.getValidInput("HTTP parameter name: " + name, name, "HTTPParameterName", 150, true);
                v.add(clean);
            } catch (ESAPIValidationException e) {
                // already logged
            }
    ...
    public String[] getParameterValues(String name) {
        ...
        for (String value : values) {
            try {
                String cleanValue = validator.getValidInput("HTTP parameter value: " + value, value, "HTTPParameterValue", HTTP_PARAMETER_VALUE_MAX, true);
    ...
    public String getPathInfo() {
        ...
        String clean = "";
        try {
            clean = validator.getValidInput("HTTP path: " + path, path, "HTTPPath", 150, true);
        } catch (ESAPIValidationException e) {
            // already logged
        }
        return clean;
    ...
    public String getQueryString() {
        ...
        String clean = "";
        try {
            clean = validator.getValidInput("HTTP query string: " + query, query, "HTTPQueryString", 2000, true);
        } catch (ESAPIValidationException e) {
            // already logged
        }
        return clean;
    ...
    public String getRequestedSessionId() {
        ...
        String clean = "";
        try {
            clean = validator.getValidInput("Requested cookie: " + id, id, "HTTPJSESSIONID", 50, false);
        } catch (ESAPIValidationException e) {
            // already logged
        }
        return clean;
    ...
    public String getRequestURI() {
        ...
        String clean = "";
        try {
            clean = validator.getValidInput("HTTP URI: " + uri, uri, "HTTPURI", 2000, false);
        } catch (ESAPIValidationException e) {
            // already logged
        }
        return clean;
    ...
    public StringBuffer getRequestURL() {
        ...
        String clean = "";
        try {
            clean = validator.getValidInput("HTTP URL: " + url, url, "HTTPURL", 2000, false);
        } catch (ESAPIValidationException e) {
            // already logged
        }
    ...
    public String getScheme() {
        ...
        String clean = "";
        try {
            clean = validator.getValidInput("HTTP scheme: " + scheme, scheme, "HTTPScheme", 10, false);
        } catch (ESAPIValidationException e) {
            // already logged
        }
        return clean;
    ...
    public String getServerName() {
        ...
        String clean = "";
        try {
            clean = validator.getValidInput("HTTP server name: " + name, name, "HTTPServerName", 100, false);
        } catch (ESAPIValidationException e) {
            // already logged
        }
        return clean;
    ...
    public String getServletPath() {
        ...
        String clean = "";
        try {
            clean = validator.getValidInput("HTTP servlet path: " + path, path, "HTTPServletPath", 100, false);
        } catch (ESAPIValidationException e) {
            // already logged
        }
        return clean;
